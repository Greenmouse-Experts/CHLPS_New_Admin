...
import {
  Certificate, CertificatesApiResponse, CertStats, CertStatsApiResponse,
  CertTemplate, TemplatesApiResponse,
...
class CertificatesRepository {
  ...
  public async update(id: string, payload: { certificateUrl: string; templateId: string; certificateNumber: string }) {
    const res = await this._api.patchData<typeof payload, { message?: string }>(ApiUrls.certificateById(id), payload);
    return { success: res.success, message: unwrapMessage(res.data, res.message || "Updated") };
  }

  public async revoke(id: string) {
    const res = await this._api.patchData<undefined, { message?: string }>(ApiUrls.revokeCertificate(id));
    return { success: res.success, message: unwrapMessage(res.data, res.message || "Revoked") };
  }

  public async remove(id: string) {
    const res = await this._api.deleteData<{ message?: string }>(ApiUrls.certificateById(id));
  ...
  public async uploadTemplate(name: string, file: File) {
    ...
    const res = await this._api.postFormData<{ message?: string }>(ApiUrls.createCertificateTemplate, fd);
    return { success: res.success, message: unwrapMessage(res.data, res.message || "Uploaded") };
  }

  public async setDefault(id: string) {
    const res = await this._api.patchData<undefined, { message?: string }>(ApiUrls.setDefaultTemplate(id));
    return { success: res.success, message: unwrapMessage(res.data, res.message || "Set as default") };
  }

  public async deleteTemplate(id: string) {
    const res = await this._api.deleteData<{ message?: string }>(ApiUrls.deleteTemplate(id));
    return { success: res.success, message: unwrapMessage(res.data, res.message || "Deleted") };
  }
}
