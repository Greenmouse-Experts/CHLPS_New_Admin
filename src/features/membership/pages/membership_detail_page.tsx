...
export default function MembershipDetailPage({
  ...
  const headerActions: Actions[] = [
    ...
    },
    {
      ...
      render: () => (
        ...
        >
          {membership?.status === "published" ? "Unpublish Plan" : "Publish Plan"}
        </span>
      ),
  ...
  const subscriberActions: Actions<MembershipSubscriber>[] = [
    ...
    },
    {
      ...
      action: (row) => {
        alert(`Renewal notice sent to ${row.name} (${row.email})`);
      },
    },
  ];
  ...
  const transactionColumns: columnType<MembershipTransaction>[] = [
    ...
    },
    {
      ...
      render: (_, row) => (
        <div>
          <p className="text-sm font-semibold text-base-content">{row.memberName}</p>
  ...
  const transactionActions: Actions<MembershipTransaction>[] = [
    {
      ...
      label: "View Receipt",
      action: (row) => {
        alert(`Receipt for ${row.reference} (${formatCurrency(row.amount, { currency: row.currency })})`);
      },
    },
  ];
  ...
  if (!membership) {
    return (
      <DashboardLayout title="Membership Details">
        <div className="bg-white rounded-xl border border-[#E7E9EB] p-12 text-center space-y-4">
          <p className="text-base font-semibold text-base-content">
            Membership Plan Not Found
          </p>
          <p className="text-sm text-base-content/60 max-w-sm mx-auto">
            The membership plan you are looking for does not exist or may have been deleted.
          </p>
    ...
    <DashboardLayout title="Membership Details">
      <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 bg-white rounded-xl border border-[#E7E9EB] p-4 shadow-sm">
          <div className="flex items-center gap-3">
            <button
              ...
              className="w-9 h-9 rounded-lg border border-[#E7E9EB] hover:bg-base-200 flex items-center justify-center text-base-content/70 transition-colors"
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs px-2.5 py-0.5 rounded-full bg-primary/10 text-primary font-medium capitalize">
                  {labelOf(MEMBERSHIP_CATEGORIES, membership.category)}
                </span>
              </div>
              <p className="text-xs text-base-content/60 mt-0.5">
                Created on {formatDate(membership.createdDate, "DD MMMM YYYY")}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <Button
              size="sm"
              variant={membership.status === "published" ? "outline" : "primary"}
              loading={isSaving}
        ...
        <div className="grid sm:grid-cols-4 gap-4">
          <StatCard
            title="Total Members"
            value={membership.membersCount}
            loading={isLoading}
            icon={<People size={20} color="#717171" />}
          />
          <StatCard
            title="Enrolled This Month"
            value={membership.membersThisMonth}
            loading={isLoading}
          ...
          <div className="bg-white rounded-xl border border-[#E7E9EB] p-4 shadow-sm flex flex-col justify-between">
            <p className="text-xs text-base-content/60 font-medium">Plan Rate</p>
            <div className="mt-2">
              <p className="text-xl font-bold text-base-content">
                {formatCurrency(membership.price, { currency: membership.currency })}
              </p>
              <p className="text-xs text-base-content/60 capitalize mt-0.5">
                Per {labelOf(MEMBERSHIP_DURATIONS, membership.duration)}
              </p>
            </div>
          </div>
        </div>
                  ...
                  <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div className="bg-base-200/40 p-3 rounded-lg border border-base-300">
                      ...
                      <p className="text-sm font-semibold text-base-content mt-0.5">
                        {labelOf(MEMBERSHIP_DURATIONS, membership.duration)}
                      </p>
                    </div>
                    <div className="bg-base-200/40 p-3 rounded-lg border border-base-300">
                    ...
                    {membership.autoRenewal && (
                      <>
                        <div className="bg-base-200/40 p-3 rounded-lg border border-base-300">
                          ...
                          <p className="text-sm font-semibold text-base-content mt-0.5">
                            {formatCurrency(membership.renewalPrice || membership.price, {
                              currency: membership.currency,
                            })}
                          </p>
                        </div>
                        <div className="bg-base-200/40 p-3 rounded-lg border border-base-300">
                          ...
                          <p className="text-sm font-semibold text-base-content mt-0.5 capitalize">
                            {membership.renewalPeriod?.replace("_", " ") || "Annual"}
                          </p>
                        </div>
                      </>
                    )}
                    <div className="bg-base-200/40 p-3 rounded-lg border border-base-300">
                      <p className="text-xs text-base-content/60">Registration Opens</p>
                      <p className="text-sm font-semibold text-base-content mt-0.5">
                        {formatDate(membership.registrationStartDate, "DD MMM YYYY")}
                      </p>
                    </div>
                    <div className="bg-base-200/40 p-3 rounded-lg border border-base-300">
                      <p className="text-xs text-base-content/60">Registration Closes</p>
                      <p className="text-sm font-semibold text-base-content mt-0.5">
                        {membership.registrationEndDate
                          ? formatDate(membership.registrationEndDate, "DD MMM YYYY")
                          : "Ongoing / Open"}
                      </p>
                    </div>
                  </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-[#E7E9EB] p-6 shadow-sm space-y-4">
                  ...
                  <div className="space-y-2.5">
                    {membership.eligibilityCriteria.map((crit, idx) => (
                      <div
                        key={idx}
                    ...
                    {membership.eligibilityCriteria.length === 0 && (
                      <p className="text-sm text-base-content/60">
                ...
                <div className="bg-white rounded-xl border border-[#E7E9EB] p-6 shadow-sm space-y-4">
                  ...
                  <div className="space-y-2">
                    {membership.requiredDocuments.map((docKey, idx) => {
                      const docLabel = labelOf(REQUIRED_DOCUMENTS, docKey);
                      return (
                        <div
                          key={idx}
                          className="flex items-center gap-3 p-3 rounded-lg border border-[#E7E9EB] bg-base-100"
                        >
                          <DocumentText size={18} className="text-base-content/70" />
                    ...
                    {membership.requiredDocuments.length === 0 && (
                      <p className="text-sm text-base-content/60">
